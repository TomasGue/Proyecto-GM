...
public class DemonNave extends Monstruo {
    private Texture disparoTextura;
    private boolean moviendoDerecha = true;
    private final int VELOCIDAD = 100; // Velocidad horizontal
    ...
    public DemonNave(Texture textura, Texture disparoTextura, float x, float y, int vidas, 
                     Sound sonidoDaño, Sound sonidoMuerte, Ataque estrategiaAtaque) {
        super(textura, x, y, vidas, estrategiaAtaque); // Llama al constructor de Monstruo con la estrategia
    ...
    public void mover(float delta) {
        ...
        if (moviendoDerecha) {
            x += VELOCIDAD * delta;
            if (x > 1200 - textura.getWidth()) { // Límite derecho
                moviendoDerecha = false;
            }
        } else {
            x -= VELOCIDAD * delta;
            if (x < 0) { // Límite izquierdo
    ...
    public void actualizar(float delta, SpriteBatch batch) {
        ...
        // Puedes agregar lógica específica de DemonNave aquí si es necesario
    }
    ...
    public void dispose() {
        if (disparoTextura != null) disparoTextura.dispose();
